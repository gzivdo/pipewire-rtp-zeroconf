 ...
 *
 * AvahiPoll → PipeWire loop adapter. Uses only the pw_loop_add_timer /
 * pw_loop_add_io API so it builds on PipeWire 1.0 (Ubuntu 24.04) as well
 * as 1.2+ (where pw_timer_queue exists).
 *
 * Adapted from upstream pipewire 1.0.5
 * (src/modules/module-zeroconf-discover/avahi-poll.c).
 */
...
{
	pw_loop_update_io(w->impl->loop, w->source, to_pw_events(event));
}
				 ...
				 AvahiTimeoutCallback callback, void *userdata)
{
	...
	t->callback = callback;
	t->userdata = userdata;
	t->source = pw_loop_add_timer(impl->loop, timeout_callback, t);
	if (t->source == NULL) {
		free(t);
		return NULL;
	}
	...
	if (tv != NULL) {
		...
		if (value.tv_sec == 0 && value.tv_nsec == 0)
			value.tv_nsec = 1;
		pw_loop_update_timer(impl->loop, t->source, &value, NULL, true);
	}
	return t;
}
...
static void timeout_update(AvahiTimeout *t, const struct timeval *tv)
{
	...
	if (tv != NULL) {
		...
		value.tv_nsec = (long) tv->tv_usec * 1000L;
		if (value.tv_sec == 0 && value.tv_nsec == 0)
			value.tv_nsec = 1;
		v = &value;
	}
	pw_loop_update_timer(t->impl->loop, t->source, v, NULL, true);
}
